			...
			if (search.trim() !== '') {
				...
				timeoutId = setTimeout(async () => {
					try {
						...
						const response = await blogService.getBlogBySearchTerm(search);
						setBlogs({ loading: false, data: response, error: null });
					} catch (error) {
	...
	return (
		<div className="px-5 md:px-0">
			...
			<div className="relative w-full items-center justify-center flex">
				<div className="w-full items-center justify-center flex  flex-col">
					<div className="w-full md:w-[70%] relative items-center justify-center flex rounded-md overflow-hidden ">
						...
						/>
						<button onClick={handelSearch} className="h-full px-5 text-xs md:text-base md:px-10 bg-[#7F56D9] text-white  absolute right-0">
							Search
						</button>
					</div>
						...
						{tags.map((tag) => (
							<button
								...
								className={`px-5 border text-textPrimary py-2 col-auto rounded-sm shadow-sm uppercase border-blue-400  hover:bg-blue-400 hover:text-white duration-150 ${
									selectedTag === tag ? 'bg-blue-400 text-white' : ''
								}`}
							>
								{tag}
							</button>
			...
			<section className=" px-5">
				<p className="text-[24px]  leading-[32px] mt-10 mb-5  text-textPrimary h-[32px] text-center py-10 md:text-left ">Search blog posts</p>
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 place-items-center lg:grid-cols-3 gap-[24px] md:gap-10">
					...
					{!blogs.error && blogs.loading ? (
						...
						blogs.data?.map((item, index) => <BlogCart key={index} item={item} />)
					)}
				</div>
			</section>
		</div>
